...
The program failed to create log files because the logs directory does not exist. As a result, a NoSuchFileException was 
thrown when I attempted to create log files, causing errors to be logged essentially to the terminal instead of being
written to a file.
...
A better data type to use instead of double is BigDecimal because it provides precise decimal representation and does a 
good job of avoiding rounding errors which double really doesn't prioritize. And this is important in financial applica
-tions where accuracy is important, as double can introduce minor precision errors due to its floating point representation.
...
There are a different number of messages printed to the console because essentially the default logging config only displays messages
really at a certain level from looking at it. Lower-level messages such as FINE, FINER, and FINEST are filtered out and not shown.
...
```text
C:\Users\abuja\.jdks\ms-21.0.10\bin\java.exe "-javaagent:C:\Users\abuja\IntelliJ IDEA 2025.3.2\lib\idea_rt.jar=62473" -Dfile.encoding=UTF-8 -Dsun.stdout.encoding=UTF-8 -Dsun.stderr.encoding=UTF-8 -classpath C:\CSC213Lab6\out\production\CSC213LAB6 config.LoggingExample
```
...
```text
C:\Users\abuja\.jdks\ms-21.0.10\bin\java.exe -Djava.util.logging.config.file=C:\CSC213Lab6\src\config\logging.properties "-javaagent:C:\Users\abuja\IntelliJ IDEA 2025.3.2\lib\idea_rt.jar=56864" -Dfile.encoding=UTF-8 -Dsun.stdout.encoding=UTF-8 -Dsun.stderr.encoding=UTF-8 -classpath C:\CSC213Lab6\out\production\CSC213LAB6 config.LoggingExample
```
...
The output is no longer displayed in the console and is instead written to a log file "captains.log". And the messages
are redirected based on the logging configuration rather than showing up in the terminal like before.

### Q3: Where is the console output being written?

The console output is being written to a log file (captains.log) located in the directory specified by the logging 
configuration (path/2/logs).
